        ...
        namespace Core {
            namespace Overlay {
                namespace Tree {
                    ...
                    void TreeOverlay::BuildOverlay(
                            std::vector<std::shared_ptr<Ripple::Common::Entity::NodeMetadata>> nodeList) {
                        this->SetNodeList(nodeList);
                        for (auto it = nodeList.begin(); it != nodeList.end(); it++) {
                            auto source = *it;
                            std::vector<std::shared_ptr<Ripple::Common::Entity::NodeMetadata>> list = this->calculateNodeListForSource(
                                    source,
                                    nodeList);
                            auto tree = std::make_shared<CompleteTree>(this->GetBranch(), list);
                            this->GetTrees().insert(
                                    std::pair<int, std::shared_ptr<CompleteTree>>(source->GetId(), tree));
                        }
                    }

                    std::vector<std::shared_ptr<Ripple::Common::Entity::NodeMetadata>>
                    TreeOverlay::calculateNodeListForSource(
                            std::shared_ptr<Ripple::Common::Entity::NodeMetadata> source,
                            std::vector<std::shared_ptr<Ripple::Common::Entity::NodeMetadata>> nodeList) {
                        std::vector<std::shared_ptr<Ripple::Common::Entity::NodeMetadata>> ret(nodeList);
                        int i = 0;
                        for (auto it = nodeList.begin(); it != nodeList.end(); it++) {
                    ...
                    TreeOverlay::generateNodeList(std::vector<std::shared_ptr<TreeNode>> treeNodeList) {
                        ...
                        for (auto it = treeNodeList.begin(); it != treeNodeList.end(); it++) {
                            ...
                            Ripple::Common::Logger::Info("TreeOverlay", "Attempting to send to node %d (%s:%d).",
                                                         nodeMetadata->GetId(),
                    ...
                    std::vector<std::shared_ptr<Ripple::Common::Entity::NodeMetadata>>
                    TreeOverlay::CalculateNodesToSync(std::shared_ptr<Ripple::Common::Entity::AbstractMessage> message,
                                                      std::shared_ptr<Ripple::Common::Entity::NodeMetadata> source,
                                                      std::shared_ptr<Ripple::Common::Entity::NodeMetadata> current) {
                        Ripple::Common::Logger::Info("TreeOverlay",
                        ...
                        Ripple::Common::Logger::Info("TreeOverlay", "I cannot find children list for node %d.",
                                                     current->GetId());
                        return std::vector<std::shared_ptr<Ripple::Common::Entity::NodeMetadata>>();
                    }

                    std::vector<std::shared_ptr<Ripple::Common::Entity::NodeMetadata>>
                    TreeOverlay::CalculateNodesToCollectAck(
                    ...
                    TreeOverlay::GetNodeList() const {
                        return this->nodeList;
                    }

                    void TreeOverlay::SetNodeList(
                            std::vector<std::shared_ptr<Ripple::Common::Entity::NodeMetadata>> nodeList) {
